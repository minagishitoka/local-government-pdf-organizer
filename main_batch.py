...
"""
============================================================
自治体議会調査 PDF整理・結合システム（Gemini Batch API版）
============================================================
...
★★★ このプログラムは何をするもの？ ★★★
  ...
  この版は通常版 main.py を残したまま、Google AI APIの無料枠でも長く使えるよう、Pythonによる事前判定・API呼び出し抑制・間隔制御・ローカル使用量カウンタを追加した版です。
...
RESERVED_OUTPUT_DIRNAMES = {"整理済み", "結合PDF", "レポート", "TXT", "Word", "整理済み_無料耐久", "結合PDF_無料耐久", "レポート_無料耐久", "TXT_無料耐久", "Word_無料耐久"}
...
class Config:
    ...
    max_ai_workers: int = 1                # 無料耐久版は同時実行しない
    ...
    free_usage_path: str = field(default_factory=lambda: os.path.join(SCRIPT_DIR, "free_api_usage.json"))
    free_retry_wait_seconds_on_429: float = 20.0
...
# =====================================================
# 3.5. 無料耐久版のAIゲート
#      Pythonで確定できる資料はGeminiに送らない
# =====================================================

FREE_API_LOCK = threading.Lock()
FREE_API_LAST_CALL = 0.0
FREE_API_CALLS_THIS_RUN = 0

def _free_load_usage(config: Config) -> dict:
    try:
        if os.path.exists(config.free_usage_path):
            with open(config.free_usage_path, "r", encoding="utf-8") as f:
                data = json.load(f)
...
def _free_save_usage(config: Config, data: dict):
    try:
        with open(config.free_usage_path, "w", encoding="utf-8") as f:
            json.dump(data, f, ensure_ascii=False, indent=2)
    except Exception as e:
        print(f"  ⚠️ 無料耐久版の使用量記録を保存できませんでした: {e}")

def _free_reserve_api_call(config: Config) -> bool:
    global FREE_API_LAST_CALL, FREE_API_CALLS_THIS_RUN
    with FREE_API_LOCK:
        usage = _free_load_usage(config)
        if FREE_API_CALLS_THIS_RUN >= config.free_max_ai_calls_per_run:
            return False
        if usage.get("calls", 0) >= config.free_daily_ai_call_limit:
            return False

        now = time.monotonic()
        wait = config.free_min_seconds_between_api_calls - (now - FREE_API_LAST_CALL)
        if wait > 0:
            time.sleep(wait)

        FREE_API_LAST_CALL = time.monotonic()
        FREE_API_CALLS_THIS_RUN += 1
        usage["calls"] = int(usage.get("calls", 0)) + 1
        _free_save_usage(config, usage)
        return True
...
def _normalize_era_date(text: str) -> str:
    ...
    year = 1 if year_s == "元" else int(year_s)
    base = {"令和": 2018, "平成": 1988, "昭和": 1925}[era]
    return f"{base + year:04d}-{int(month_s):02d}-{int(day_s):02d}"
...
def _offline_extract_text(path: str, max_pages: int = 3) -> str:
    if fitz is None:
        return ""
    doc = fitz.open(path)
    try:
...
def _offline_municipality(text: str) -> str:
...
def _offline_date(text: str) -> str:
    lines = [unicodedata.normalize("NFKC", x).strip() for x in text.splitlines() if x.strip()]
    scored = []
    ...
    for idx, line in enumerate(lines):
        ...
        for pat in date_patterns:
            dates.extend(pat.findall(line))
        # findall with capturing groups can return tuples for era pattern, so use finditer below.
        matches = []
        for pat in date_patterns:
            matches.extend(pat.finditer(line))
        for m in matches:
            raw = m.group(0)
            normalized = _normalize_era_date(raw) or _normalize_gregorian_date(raw)
            if not normalized:
                continue
            score = 0
            context = line
            if any(w in context for w in good_words):
                score += 4
            if any(w in context for w in bad_words):
                score -= 5
            if idx <= 2:
                score += 1
            scored.append((score, normalized))
    unique = []
    for score, value in sorted(scored, key=lambda x: x[0], reverse=True):
        if value not in unique:
            unique.append(value)
    if not scored:
        return ""
    top_score = max(s for s, _ in scored)
    top = [v for s, v in scored if s == top_score]
...
def _offline_extract_metadata(path: str, config: Config) -> tuple:
    text = _offline_extract_text(path, max_pages=3)
    municipality = _offline_municipality(text)
    document_date = _offline_date(text)
    title = _offline_title_candidate(text, os.path.basename(path))
    return municipality, document_date, title, text

def _free_should_call_ai(municipality: str, document_date: str, title: str) -> bool:
    # 3項目すべてがPythonで明確に取れた場合だけAIを省略する。
    return not (municipality and document_date and title)
...
BATCH_INPUT_DIR = os.path.join(BATCH_DIR, "input")
BATCH_PDF_DIR = os.path.join(BATCH_DIR, "pdf")
BATCH_RESULT_DIR = os.path.join(BATCH_DIR, "results")
BATCH_STATE_PATH = os.path.join(BATCH_DIR, "batch_state.json")
...
def _batch_ensure_dirs():
    for path in (BATCH_DIR, BATCH_INPUT_DIR, BATCH_PDF_DIR, BATCH_RESULT_DIR):
        os.makedirs(path, exist_ok=True)

def _batch_jsonable(value):
    if value is None or isinstance(value, (str, int, float, bool)):
        return value
    if isinstance(value, dict):
        return {k: _batch_jsonable(v) for k, v in value.items()}
    if isinstance(value, (list, tuple)):
        return [_batch_jsonable(v) for v in value]
    if hasattr(value, "model_dump"):
        try:
            return _batch_jsonable(value.model_dump(exclude_none=True))
        except Exception:
            pass
    if hasattr(value, "to_dict"):
        try:
            return _batch_jsonable(value.to_dict())
        except Exception:
            pass
    raise TypeError(f"Batch JSON化できない値: {type(value)!r}")

def _batch_extract_text(response_obj):
    if response_obj is None:
        return ""
    if isinstance(response_obj, dict):
        if response_obj.get("text"):
...
def _batch_prepare_input(records, client, config):
    ...
    jsonl_path = os.path.join(BATCH_INPUT_DIR, f"batch_input_{timestamp}.jsonl")
    upload_manifest = {}
    requests = 0
...
def _batch_create_or_resume(client, records, config):
    state = _batch_load_state()
    active_name = state.get("batch_name")
    if active_name:
        try:
            job = client.batches.get(name=active_name)
            if getattr(job, "state", None) not in BATCH_TERMINAL_STATES:
                return job, state
    ...
    input_name, request_count, manifest = _batch_prepare_input(records, client, config)
    if not input_name:
        state = {
            "status": "NO_BATCH_NEEDED",
            "request_count": 0,
            "updated_at": datetime.now().isoformat(),
        }
        _batch_save_state(state)
        return None, state
    ...
    job = client.batches.create(
        ...
        src=input_name,
        config={
            "display_name": f"local-government-pdf-{datetime.now().strftime('%Y%m%d-%H%M%S')}",
        },
    )
    state = {
        "batch_name": job.name,
        "status": getattr(job, "state", "JOB_STATE_PENDING"),
...
def _batch_download_results(client, job, state):
    """
    成功したBatchの出力JSONLを取得する。
    SDKの出力表現差に備えて、まずdest.file_nameを探す。
    """
    dest = getattr(job, "dest", None)
    file_name = getattr(dest, "file_name", None) if dest else None
    if not file_name and isinstance(dest, dict):
        file_name = dest.get("file_name")
    if not file_name:
        raise RuntimeError("Batch成功済みですが、出力ファイル名(dest.file_name)を取得できません")
...
def _batch_apply_results(records, result_path, config):
    ...
    with open(result_path, "r", encoding="utf-8") as f:
        for line_no, line in enumerate(f, 1):
            ...
            rec = by_key.get(key)
            if rec is None:
                print(f"  ⚠️ Batch結果{line_no}行目: 対応するPDFが見つかりません: {key}")
                failed += 1
                continue
            ...
            if error:
                ...
                rec.ai_error = str(error)
                rec.final_title = rec.original_filename
                rec.validation_status = "needs_review"
                rec.validation_reason = "Gemini Batch APIで個別リクエストが失敗"
                failed += 1
                continue

            raw_text = _batch_extract_text(response)
            try:
                municipality_raw, date_raw, raw_title, teasing = parse_ai_json(raw_text)
                ...
                municipality, municipality_status, municipality_reason = validate_municipality(municipality_raw)
                document_date, date_status, date_reason = validate_document_date(date_raw)
                final_title, title_status, title_reason = validate_ai_title(
                    raw_title, rec.original_filename, config
                )
                rec.municipality = municipality
                rec.document_date = document_date
                ...
                rec.validation_status = (
                    "ok"
                    if municipality_status == "ok" and date_status == "ok" and title_status == "ok"
                    else "needs_review"
                )
                rec.validation_reason = "; ".join(
                    x for x in (municipality_reason, date_reason, title_reason) if x
                )
                applied += 1
            except Exception as e:
                rec.ai_status = "failed"
                ...
                rec.final_title = rec.original_filename
                rec.validation_status = "needs_review"
                rec.validation_reason = "Batch結果のJSON解析または検証に失敗"
                failed += 1
...
def _batch_wait_and_apply(client, job, records, state, config):
    ...
    while True:
        ...
        _batch_save_state(state)
        print(f"  🛰️ Batch状態: {current} / {state.get('request_count', 0)}件")
        if current in BATCH_TERMINAL_STATES:
            break
        time.sleep(poll)
    ...
    state["status"] = current
    _batch_save_state(state)
    print(f"  📥 Batch結果取得: 成功適用{applied}件 / 失敗{failed}件")
    return state
...
def process_single_pdf(path: str, client: "genai.Client", config: Config) -> PdfRecord:
    ...
    if rec.inspect_status != "ok":
        with print_lock:
            print(f"  ❌ [開けない] {rec.original_filename}: {rec.inspect_error}")
        return rec

    # 無料耐久版の核心:
    # まずPythonで冒頭3ページを読む。明確な資料ならGeminiを一切呼ばない。
    offline_municipality, offline_date, offline_title, offline_text = _offline_extract_metadata(path, config)

    if config.free_mode and not _free_should_call_ai(
        offline_municipality, offline_date, offline_title
    ):
        rec.ai_municipality_raw = offline_municipality
        rec.municipality = offline_municipality
        rec.ai_date_raw = offline_date
        rec.document_date = offline_date
        rec.ai_title_raw = offline_title
        rec.final_title = offline_title
        rec.ai_teasing = ""
        rec.ai_status = "skipped_python_clear"
    ...
    try:
        ...
        municipality_raw, date_raw, raw_title, teasing = _free_call_ai_or_raise(
            client, title_pages_bytes, config
        )
        rec.ai_municipality_raw = municipality_raw
        rec.ai_date_raw = date_raw
        rec.ai_title_raw = raw_title
        ...
        rec.ai_teasing = ""
        rec.validation_status = "needs_review"
        rec.validation_reason = "AI呼び出し失敗または無料耐久版のローカル上限到達"
        with print_lock:
            print(f"  ⚠️ [AI要確認] {rec.original_filename}: {e}")
        return rec
