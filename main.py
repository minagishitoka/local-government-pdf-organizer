...
- 会議・委員会・総会・審議会などの「会議そのもの」を特定するために、
  表紙に記載された開催年月日・開催日・回次は、タイトルの一部として必ず残す。
- 例：「令和〇年〇月〇日 〇〇委員会」
  「第〇回 〇〇会議（令和〇年〇月〇日開催）」のように、
  会議名と開催日を組み合わせて資料を識別できる形にする。
- 一方、単なる発行日・作成日・更新日・印刷日など、
  会議の開催日ではない日付は title に入れない。
...
def parse_ai_json(raw_text: str) -> tuple[str, str]:
...
def process_single_pdf(path: str, client: "genai.Client", config: Config) -> PdfRecord:
    ...
    try:
        ...
        raw_title, teasing = call_ai_title_and_teasing(client, title_pages_bytes, config)
    ...
    final_title, status, reason = validate_ai_title(
...
def copy_and_rename(records: list, renamed_dir: str) -> None:
    ...
    for rec in records:
        ...
        title_for_filename = rec.final_title
        if len(title_for_filename) > max_name_len:
            title_for_filename = title_for_filename[: max_name_len - 1] + "…"
            if rec.validation_status == "ok":
                rec.validation_status = "needs_review"
            reason = "ファイルパスが長くなりすぎるため、ファイル名のみ短縮"
            rec.validation_reason = (rec.validation_reason + "; " + reason).strip("; ")

        new_name = build_unique_filename(title_for_filename, ext, used_names)
...
def build_toc_and_assign_pages(group_records: list, volume_index: int):
    ...
    def draw_page_header(is_first_page: bool):
        ...
        can.drawString(TOC_LEFT_MARGIN, TOC_HEADER_Y, "No.  [元ファイル名] 資料タイトル")
    ...
    for i, rec in enumerate(group_records, 1):
        ...
        left_text_full = f"{i}. [{rec.original_filename}] {rec.final_title}"
...
def txt_to_word(txt_path: str, word_dir: str, expected_records: Optional[list] = None):
    ...
    # 目次を先頭に置く。資料タイトルは「【資料 xxx】」直後の「資料タイトル:」から取得。
    toc_items = []
    for i, line in enumerate(lines):
        if line.startswith("【資料 ") and i + 1 < len(lines):
...
def write_reports(records: list, report_dir: str):
    ...
    fieldnames = [
        "総合ステータス", "元ファイル名", "整理後ファイル名", "資料タイトル",
...
def main():
    ...
    for path in pdf_paths:
        ...
        if key in state:
            ...
            records.append(rec)
            with print_lock:
                print(f"  ⏭️ [スキップ] {rec.original_filename} （これ前に終わらせてあるやつだな。飛ばすぞ）")
        else:
